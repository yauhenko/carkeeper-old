...
export default class CarMenu extends React.Component {
  ...
  render() {
    return (
      <React.Fragment>
        <Button vertical style={this.activeStyle("Journal")} onPress={()=>this.props.navigation.navigate("Journal", {id: this.id})} active={this.activeButton("Journal")}>
          <Icon style={styles.footerBadge} name={"clipboard"} />
          <Text style={{color: "#fff"}}>Бортжурнал</Text>
        </Button>

        <Button vertical style={this.activeStyle("Reminders")} onPress={()=>this.props.navigation.navigate("Reminders", {id: this.id})} active={this.activeButton("Reminders")}>
          <Icon style={styles.footerBadge} name={"watch"} />
          <Text style={{color: "#fff"}}>Напоминания</Text>
        </Button>

        <Button badge vertical style={this.activeStyle("Fines")} onPress={()=>this.props.navigation.navigate("Fines", {id: this.id})} active={this.activeButton("Fines")}>
